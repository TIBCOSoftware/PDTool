...
writeOutput "------------------------------------------------------------------"                             "" $SEP -nodate
writeOutput "-----------                                            -----------"                             "" $SEP -nodate
writeOutput "----------- TIBCO Professional Services               -----------"                              "" $SEP -nodate
...
#=======================================
if [ "$CIS_VERSION" == "" ]; then
    writeOutput "The variable CIS_VERSION is not set properly from cisVersion.sh"                            $SCRIPT $SEP
    exit 1
fi
...
ENDORSED_DIR="${PROJECT_HOME}/lib/tdv/endorsed"
JAVA_ENDORSED_DIRS="-Djava.endorsed.dirs=\"${ENDORSED_DIR}\""
DEPLOY_CLASSPATH="${PROJECT_HOME}/dist/*:${PROJECT_HOME}/lib/pdtool/*:${PROJECT_HOME}/lib/common/*:${PROJECT_HOME}/lib/tdv/*"

CIS_VERSION_NODOT=`echo $CIS_VERSION | sed -e 's/\.//g'`
if [ "$CIS_VERSION_NODOT" -ge "830" ]; then
    JAVA_ENDORSED_DIRS=""
    DEPLOY_CLASSPATH="${PROJECT_HOME}/dist/*:${PROJECT_HOME}/lib/pdtool/*:${PROJECT_HOME}/lib/common/*:${PROJECT_HOME}/lib/tdv/*:${ENDORSED_DIR}/*"
    JAVA_OPT="${AVA_OPT} --add-exports=java.base/sun.security.provider=ALL-UNNAMED --add-exports=java.security.jgss/sun.security.krb5=ALL-UNNAMED --add-opens=java.base/sun.nio.ch=ALL-UNNAMED --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED --add-opens=java.base/java.lang=ALL-UNNAMED --add-opens=jdk.management/com.sun.management.internal=ALL-UNNAMED --add-exports=java.naming/com.sun.jndi.ldap=ALL-UNNAMED --add-exports=java.base/sun.nio.ch=ALL-UNNAMED --add-exports=java.base/jdk.internal.ref=ALL-UNNAMED --add-exports=java.base/sun.net.www.protocol.file=ALL-UNNAMED --add-opens=java.base/java.io=ALL-UNNAMED"
fi

# NOTE: the above JAVA_OPT suppresses the following message as a result of using JDK 11.  The message does not happen with JRE 1.8
# WARNING: An illegal reflective access operation has occurred
# WARNING: Illegal reflective access by com.sun.xml.bind.v2.runtime.reflect.opt.Injector$1 (file:/C:/MyFiles/PDTool/PDTool8.3.0_20201014/PDTool/lib/tdv/webservices-rt-2.1.jar) to method java.lang.ClassLoader.defineClass(java.lang.String,byte[],int,int)
...
# Test for VCS_USERNAME and VCS_PASSWORD
PR_VCS_PASSWORD=""
...
#=======================================
writeOutput "########################################################################################################################################" "" $SEP -nodate
writeOutput "$SCRIPT: Command line resolved variables:"                                                      "" $SEP -nodate
writeOutput "########################################################################################################################################" "" $SEP -nodate
writeOutput "Resolved Arguments:"                                                                            $SCRIPT $SEP -nodate
writeOutput "   CMD=[$CMD]"                                                                                  $SCRIPT $SEP -nodate
writeOutput "   MY_VARS_PATH=[$MY_VARS_PATH]"                                                                $SCRIPT $SEP -nodate
writeOutput "   PROPERTY_FILE=[$PROPERTY_FILE]"                                                              $SCRIPT $SEP -nodate
writeOutput "   ENCRYPT_BYPASS_STRING=[$ENCRYPT_BYPASS_STRING]"                                              $SCRIPT $SEP -nodate
writeOutput "   VCS_USERNAME=[$VCS_USERNAME]"                                                                $SCRIPT $SEP -nodate
writeOutput "   VCS_PASSWORD=[$PR_VCS_PASSWORD]"                                                             $SCRIPT $SEP -nodate
writeOutput "   CONFIG_PROPERTY_FILE=[$CONFIG_PROPERTY_FILE]"                                                $SCRIPT $SEP -nodate
writeOutput "   RELEASE_FOLDER=[$RELEASE_FOLDER]"                                                            $SCRIPT $SEP -nodate
writeOutput "   CIS_VERSION=[$CIS_VERSION]"                                                                  $SCRIPT $SEP -nodate

#=======================================
# Print the Environment Variables
#=======================================
writeOutput "########################################################################################################################################" "" $SEP -nodate
writeOutput "$SCRIPT: DeployManager Environment Variables:"                                                  "" $SEP -nodate
writeOutput "########################################################################################################################################" "" $SEP -nodate
writeOutput "   PROJECT_HOME=[$PROJECT_HOME]"                                                                $SCRIPT $SEP -nodate
writeOutput "   PROJECT_HOME_PHYSICAL=[$PROJECT_HOME_PHYSICAL]"                                              $SCRIPT $SEP -nodate
writeOutput "   ENDORSED_DIR=[$ENDORSED_DIR]"                                                                $SCRIPT $SEP -nodate
writeOutput "   JAVA_ENDORSED_DIRS=[$JAVA_ENDORSED_DIRS]"                                                    $SCRIPT $SEP -nodate
writeOutput "   DEPLOY_CLASSPATH=[$DEPLOY_CLASSPATH]"                                                        $SCRIPT $SEP -nodate
writeOutput "   DEPLOY_MANAGER=[$DEPLOY_MANAGER]"                                                            $SCRIPT $SEP -nodate
writeOutput "   DEPLOY_COMMON_UTIL=[$DEPLOY_COMMON_UTIL]"                                                    $SCRIPT $SEP -nodate
writeOutput "   CONFIG_LOG4J=[$CONFIG_LOG4J]"                                                                $SCRIPT $SEP -nodate
writeOutput "   CONFIG_ROOT=[$CONFIG_ROOT]"                                                                  $SCRIPT $SEP -nodate
...
if [ "${CMD}" == "-exec" ]; then
    ...
    JAVA_ACTION="execCisDeployTool"
      COMMAND="\"$JAVA_HOME/bin/java\" ${JAVA_OPT} -classpath \"${DEPLOY_CLASSPATH}\" ${CONFIG_ROOT} ${CONFIG_LOG4J} ${JAVA_ENDORSED_DIRS} -DPROJECT_HOME=\"${PROJECT_HOME}\" -DPROJECT_HOME_PHYSICAL=\"${PROJECT_HOME_PHYSICAL}\" -DCONFIG_PROPERTY_FILE=${CONFIG_PROPERTY_FILE} ${DEPLOY_MANAGER} ${JAVA_ACTION} \"${PROPERTY_FILE}\" \"${VCS_USERNAME}\" \"${VCS_PASSWORD}\""
    PRCOMMAND="\"$JAVA_HOME/bin/java\" ${JAVA_OPT} -classpath \"${DEPLOY_CLASSPATH}\" ${CONFIG_ROOT} ${CONFIG_LOG4J} ${JAVA_ENDORSED_DIRS} -DPROJECT_HOME=\"${PROJECT_HOME}\" -DPROJECT_HOME_PHYSICAL=\"${PROJECT_HOME_PHYSICAL}\" -DCONFIG_PROPERTY_FILE=${CONFIG_PROPERTY_FILE} ${DEPLOY_MANAGER} ${JAVA_ACTION} \"${PROPERTY_FILE}\" \"${VCS_USERNAME}\" \"${PR_VCS_PASSWORD}\""
fi
...
if [ "${CMD}" == "-vcsinit" ]; then
    ...
    JAVA_ACTION="vcsInitWorkspace"
      COMMAND="\"$JAVA_HOME/bin/java\" ${JAVA_OPT} -classpath \"${DEPLOY_CLASSPATH}\" ${CONFIG_ROOT} ${CONFIG_LOG4J} ${JAVA_ENDORSED_DIRS} -DPROJECT_HOME=\"${PROJECT_HOME}\" -DPROJECT_HOME_PHYSICAL=\"${PROJECT_HOME_PHYSICAL}\" -DCONFIG_PROPERTY_FILE=${CONFIG_PROPERTY_FILE} ${DEPLOY_MANAGER} ${JAVA_ACTION} \"${VCS_USERNAME}\" \"${VCS_PASSWORD}\""
    PRCOMMAND="\"$JAVA_HOME/bin/java\" ${JAVA_OPT} -classpath \"${DEPLOY_CLASSPATH}\" ${CONFIG_ROOT} ${CONFIG_LOG4J} ${JAVA_ENDORSED_DIRS} -DPROJECT_HOME=\"${PROJECT_HOME}\" -DPROJECT_HOME_PHYSICAL=\"${PROJECT_HOME_PHYSICAL}\" -DCONFIG_PROPERTY_FILE=${CONFIG_PROPERTY_FILE} ${DEPLOY_MANAGER} ${JAVA_ACTION} \"${VCS_USERNAME}\" \"${PR_VCS_PASSWORD}\""
fi
...
if [ "${CMD}" == "-encrypt" ]; then
    ...
    JAVA_ACTION="encryptPasswordsInFileBypass"
      COMMAND="\"$JAVA_HOME/bin/java\" ${JAVA_OPT} -classpath \"${DEPLOY_CLASSPATH}\" ${CONFIG_ROOT} ${CONFIG_LOG4J} ${JAVA_ENDORSED_DIRS} -DPROJECT_HOME=\"${PROJECT_HOME}\" -DPROJECT_HOME_PHYSICAL=\"${PROJECT_HOME_PHYSICAL}\" -DCONFIG_PROPERTY_FILE=${CONFIG_PROPERTY_FILE} ${DEPLOY_COMMON_UTIL} ${JAVA_ACTION} \"${PROPERTY_FILE}\" \"${ENCRYPT_BYPASS_STRING}\""
    PRCOMMAND="${COMMAND}"
fi
...
if [ "${CMD}" == "-ant" ]; then
    ...
    # Set the default Ant options
    export ANT_OPTS="$CONFIG_LOG4J $CONFIG_ROOT ${JAVA_ENDORSED_DIRS}"

    # Set the default Ant classpath
    ANT_CLASSPATH="${PROJECT_HOME}/dist:${PROJECT_HOME}/lib:${PROJECT_HOME}/lib/endorsed:${ANT_HOME}/lib"

    # Set the ANT options and ANT classpath for TDV 8.3 and higher
    if [ "$CIS_VERSION_NODOT" -ge "830" ]; then
       ANT_CLASSPATH="${PROJECT_HOME}/dist:${PROJECT_HOME}/lib/pdtool:${PROJECT_HOME}/lib/common:${PROJECT_HOME}/lib/tdv:${ENDORSED_DIR}:${ANT_HOME}/lib"
       export ANT_OPTS="$CONFIG_LOG4J $CONFIG_ROOT --add-exports=java.base/sun.security.provider=ALL-UNNAMED --add-exports=java.security.jgss/sun.security.krb5=ALL-UNNAMED --add-opens=java.base/sun.nio.ch=ALL-UNNAMED --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED --add-opens=java.base/java.lang=ALL-UNNAMED --add-opens=jdk.management/com.sun.management.internal=ALL-UNNAMED --add-exports=java.naming/com.sun.jndi.ldap=ALL-UNNAMED --add-exports=java.base/sun.nio.ch=ALL-UNNAMED --add-exports=java.base/jdk.internal.ref=ALL-UNNAMED --add-exports=java.base/sun.net.www.protocol.file=ALL-UNNAMED --add-opens=java.base/java.io=ALL-UNNAMED"
    fi
