...
#  # checkout.sh [CIS-resource-path] [CIS-resource-type] [rollback-revision] [vcs-workspace-project-folder] [vcs-temp-folder]
#  # 
#  # Parameters
#  # ----------
...
# Convert Revision to upper case
revUpper=`echo $Revision | tr [a-z] [A-Z]`
if [[ $revUpper = "HEAD" ]]; then
    Revision="HEAD"
fi
...
#  #***********************************************
JAVA_ACTION=vcsStudioCheckout
COMMAND="\"${JAVA_HOME}\"/bin/java ${JAVA_OPT} -classpath \"${DEPLOY_CLASSPATH}\" ${CONFIG_ROOT} ${CONFIG_LOG4J} -Djava.endorsed.dirs=\"${ENDORSED_DIR}\" -DPROJECT_HOME=\"${PROJECT_HOME}\" -DCONFIG_PROPERTY_FILE=\"${CONFIG_PROPERTY_FILE}\" ${DEPLOY_MANAGER} ${JAVA_ACTION} \"${ResPath}\" \"${ResType}\" \"${Revision}\" \"${Workspace}\" \"${VcsTemp}\""
#  #=======================================
#  # Execute the script
#  #=======================================
