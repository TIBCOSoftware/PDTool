...
REM echo ARG2=[%2]
	...
	:CONTINUE14
     if "%1"=="-vcsCheckinOptions" GOTO VCSCHECKINOPTIONS
 	GOTO CONTINUE15
	:VCSCHECKINOPTIONS
			set ARG=vcsCheckinOptions
            set VCS_CHECKIN_OPTIONS=%ARG2X%
			if not defined VCS_CHECKIN_OPTIONS (
			   set error=1
			) else (
				REM removed double quotes from around the parameter
			   	set VCS_CHECKIN_OPTIONS=%VCS_CHECKIN_OPTIONS:"=%
			)
            shift
            GOTO:LOOPEND
	...
	:CONTINUE15
     if "%1"=="-vcsScriptBinFolderOverride" GOTO VCSSCRIPTBINFOLDEROVERRIDE
 	GOTO CONTINUE16
	:VCSSCRIPTBINFOLDEROVERRIDE
			set ARG=vcsScriptBinFolderOverride
            set VCS_SCRIPT_BIN_FOLDER_OVERRIDE=%ARG2X%
			if not defined VCS_SCRIPT_BIN_FOLDER_OVERRIDE (
			   set error=1
			) else (
				REM removed double quotes from around the parameter
			   	set VCS_SCRIPT_BIN_FOLDER_OVERRIDE=%VCS_SCRIPT_BIN_FOLDER_OVERRIDE:"=%
			)
            shift
            GOTO:LOOPEND
...
echo VCS_PASSWORD=%PR_VCS_PASSWORD%
echo CUSTOM_CIS_PATH_LIST=%CUSTOM_CIS_PATH_LIST%
echo VCS_CHECKIN_OPTIONS=%VCS_CHECKIN_OPTIONS%
...
set   COMMAND="%JAVA_HOME%\bin\java" %JAVA_OPT% -cp  %DEPLOY_CLASSPATH% %CONFIG_ROOT% %CONFIG_LOG4J% -Djava.endorsed.dirs="%ENDORSED_DIR%" -DPROJECT_HOME="%PROJECT_HOME%" -DCONFIG_PROPERTY_FILE=%CONFIG_PROPERTY_FILE% %DEPLOY_MANAGER% %JAVA_ACTION% "%CUSTOM_CIS_PATH_LIST%" "%VCS_CHECKIN_OPTIONS%" "%VCS_USERNAME%" "%VCS_PASSWORD%"
set PRCOMMAND="%JAVA_HOME%\bin\java" %JAVA_OPT% -cp  %DEPLOY_CLASSPATH% %CONFIG_ROOT% %CONFIG_LOG4J% -Djava.endorsed.dirs="%ENDORSED_DIR%" -DPROJECT_HOME="%PROJECT_HOME%" -DCONFIG_PROPERTY_FILE=%CONFIG_PROPERTY_FILE% %DEPLOY_MANAGER% %JAVA_ACTION% "%CUSTOM_CIS_PATH_LIST%" "%VCS_CHECKIN_OPTIONS%" "%VCS_USERNAME%" "%PR_VCS_PASSWORD%"
GOTO START_SCRIPT
...
echo WINDOWS_LOGIN=%WINDOWS_LOGIN%
IF NOT DEFINED WINDOWS_LOGIN (
  set ARG=winlogin
  call :USAGE
  exit /B 1
...
echo INCLUDE_RESOURCE_SECURITY=%INCLUDE_RESOURCE_SECURITY%
IF NOT DEFINED INCLUDE_RESOURCE_SECURITY (
  set ARG=includeResourceSecurity
  call :USAGE
  exit /B 1
)
echo VCS_WORKSPACE_PATH_OVERRIDE=%VCS_WORKSPACE_PATH_OVERRIDE%
IF NOT DEFINED VCS_WORKSPACE_PATH_OVERRIDE (
  set VCS_WORKSPACE_PATH_OVERRIDE= 
)
echo VCS_SCRIPT_BIN_FOLDER_OVERRIDE=%VCS_SCRIPT_BIN_FOLDER_OVERRIDE%
IF NOT DEFINED VCS_SCRIPT_BIN_FOLDER_OVERRIDE (
...
echo PROPERTY_FILE_PATH=%PROPERTY_FILE_PATH%
